...
const BookingForm = () => {
  ...
  const handleWhatsApp = () => {
    const { name, phone, pickup, date, destination } = formData;
    
    // Constructing the message with user details
    const message = `Hi Roameo Rides! I'd like to plan a journey.%0A%0A` +
      `*Name:* ${name || "Not provided"}%0A` +
      `*Phone:* ${phone || "Not provided"}%0A` +
      `*Pick Up:* ${pickup || "Not provided"}%0A` +
      `*Date:* ${date || "Not provided"}%0A` +
    ...
    <section className="bg-surface-low py-24 px-6">
      ...
      <div className="max-w-2xl mx-auto bg-surface-container rounded-2xl p-8 md:p-12 border border-white/5 shadow-2xl">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
          {/* Name */}
          <div>
            <label className="text-[10px] uppercase tracking-[0.2em] text-on-surface-variant font-bold mb-2 block">Your Name</label>
            <input 
              name="name" 
              value={formData.name} 
          ...
          {/* Phone */}
          <div>
            <label className="text-[10px] uppercase tracking-[0.2em] text-on-surface-variant font-bold mb-2 block">Phone Number</label>
            <input 
              name="phone" 
          ...
          {/* Pickup */}
          <div>
            <label className="text-[10px] uppercase tracking-[0.2em] text-on-surface-variant font-bold mb-2 block">Pick Up Location</label>
            <input 
              name="pickup" 
          ...
          {/* Date */}
          <div>
            <label className="text-[10px] uppercase tracking-[0.2em] text-on-surface-variant font-bold mb-2 block">Travel Date</label>
            <input 
              name="date" 
              value={formData.date} 
              onChange={handleChange} 
          ...
          <div className="md:col-span-2">
            <label className="text-[10px] uppercase tracking-[0.2em] text-on-surface-variant font-bold mb-2 block">Destination</label>
            <input 
              name="destination" 
