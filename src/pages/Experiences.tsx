  ...
  <div>
    <HeroSection
      ...
      actions={[
        ...
        { label: "Explore Packages", variant: "outline" },
      ]}
    />
      ...
      <section key={i} className={`py-24 px-6 md:px-16 ${i % 2 === 0 ? "bg-background" : "bg-surface-low"}`}>
        <div className={`max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-12 items-center ${s.reverse ? "md:direction-rtl" : ""}`}>
          <div className={s.reverse ? "md:order-2" : ""}>
            ...
            <button className="text-secondary font-semibold text-sm flex items-center gap-1 hover:gap-2 transition-all uppercase tracking-widest">
              {s.cta} <ArrowRight size={14} />
            </button>
          </div>
          <div className={`h-[400px] md:h-[500px] rounded-2xl overflow-hidden ${s.reverse ? "md:order-1" : ""}`}>
